...
export function useOrderSocket(orderId: string | null, onUpdate: (data: any) => void) {
  useEffect(() => {
    if (!orderId) return;
    const socket = getSocket();
...
export function useAdminSocket(handlers: {
  ...
  useEffect(() => {
    ...
    if (handlers.onOrderUpdated) socket.on('order:updated', handlers.onOrderUpdated);
    if (handlers.onNewContact) socket.on('contact:new', handlers.onNewContact);
    if (handlers.onProductUpdated) socket.on('product:updated', handlers.onProductUpdated);
    return () => {
      if (handlers.onNewOrder) socket.off('order:new', handlers.onNewOrder);
      if (handlers.onOrderUpdated) socket.off('order:updated', handlers.onOrderUpdated);
      if (handlers.onNewContact) socket.off('contact:new', handlers.onNewContact);
      if (handlers.onProductUpdated) socket.off('product:updated', handlers.onProductUpdated);
    };
  }, []);
}
