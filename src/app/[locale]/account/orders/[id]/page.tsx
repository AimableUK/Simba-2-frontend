...
import { CheckCircle, Circle, Package, Truck, Home, Clock, MapPin } from 'lucide-react';
...
export default function OrderDetailPage() {
  ...
  if (!order) return (
    <div className="container mx-auto px-4 py-20 text-center">
      ...
      <Link href={`/${locale}/account/orders`} className="mt-4 inline-block text-primary hover:underline">← {t('title')}</Link>
    </div>
  );
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        <Link href={`/${locale}/account/orders`} className="text-muted-foreground hover:text-primary transition-colors text-sm">
          ← {t('title')}
        </Link>
      </div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold">{t('orderNumber')}{order.orderNumber}</h1>
        ...
        <div className="text-right">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 capitalize">{order.paymentStatus}</p>
        </div>
      </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 mb-4">
          ...
          <div className="relative">
            ...
            <div
              ...
              style={{ height: `${Math.min(100, (currentStep / (ORDER_STATUS_STEPS.length - 1)) * 100)}%` }}
            />
              ...
              {ORDER_STATUS_STEPS.map((step, idx) => {
                ...
                return (
                  <div key={step} className="flex gap-4 relative">
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 z-10 transition-colors border-2 ${
      ...
      <div className="bg-card border border-border rounded-2xl p-6 mb-4">
            ...
            <div key={item.id} className="flex gap-3 items-center">
              <div className="relative w-14 h-14 rounded-xl overflow-hidden bg-muted border border-border shrink-0">
                {item.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-muted-foreground text-xs">{item.name[0]}</div>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm line-clamp-1">{item.name}</p>
                <p className="text-xs text-muted-foreground">x{item.quantity} · {formatPrice(item.price)} each</p>
              </div>
              <span className="font-semibold text-sm shrink-0">{formatPrice(item.price * item.quantity)}</span>
            </div>
          ))}
        </div>
        <div className="border-t border-border mt-4 pt-4 space-y-1.5 text-sm">
          ...
          <div className="flex justify-between text-muted-foreground"><span>Delivery</span><span>{formatPrice(order.deliveryFee)}</span></div>
      ...
      <div className="bg-card border border-border rounded-2xl p-6">
        ...
        <div className="text-sm text-muted-foreground space-y-1">
          ...
          <p>{addr?.phone}</p>
          <p>{addr?.street}</p>
          <p>{addr?.district}{addr?.sector ? `, ${addr.sector}` : ''}</p>
