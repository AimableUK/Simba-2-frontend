...
const STATUSES = ['', 'pending', 'confirmed', 'packaged', 'on_the_way', 'delivered', 'cancelled'];
...
export default function AdminOrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => orderApi.adminList({ page, limit: 20, search, status: statusFilter }).then((r) => r.data),
  });
              ...
              {isLoading
                ...
                : data?.data?.map((order: Order) => (
                  <tr key={order.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs font-medium px-2.5 py-1.5 rounded-full ${STATUS_COLORS[order.status]}`}>
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedOrder(null)}>
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-muted-foreground mb-6">{selectedOrder.orderNumber}</p>

            <div className="space-y-4">
              {/* Customer info */}
              <div className="bg-muted/50 rounded-xl p-4 text-sm space-y-1">
                <p><span className="text-muted-foreground">Customer:</span> <span className="font-medium">{selectedOrder.user?.name}</span></p>
                <p><span className="text-muted-foreground">Email:</span> {selectedOrder.user?.email}</p>
                <p><span className="text-muted-foreground">Phone:</span> {selectedOrder.user?.phone || (selectedOrder.deliveryAddress as any)?.phone}</p>
                <p><span className="text-muted-foreground">Total:</span> <span className="font-bold text-primary">{formatPrice(selectedOrder.total)}</span></p>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  onClick={() => statusMutation.mutate({ id: selectedOrder.id, status: newStatus, note })}
                  disabled={statusMutation.isPending || newStatus === selectedOrder.status}
                ...
                <button onClick={() => setSelectedOrder(null)} className="px-5 py-3 border border-border rounded-xl hover:bg-muted transition-colors font-medium">
                  Cancel
                </button>
              </div>
            </div>
