          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                {[t("cols.user"), t("cols.email"), t("cols.phone"), t("cols.role"), t("cols.orders"), t("cols.joined"), t("cols.actions")].map(h => (
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setSelectedUser(null)}>
          <div className="bg-card border border-border rounded-2xl p-6 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3 mb-6">
              {ROLES.map(role => (
                ...
                >
                  ...
                  {role === 'super_admin' && <span className="ml-auto text-xs text-muted-foreground">{t("roles.super_admin")}</span>}
                  {role === 'admin' && <span className="ml-auto text-xs text-muted-foreground">{t("roles.admin")}</span>}
                  {role === 'poster' && <span className="ml-auto text-xs text-muted-foreground">{t("roles.poster")}</span>}
                  {role === 'branch_manager' && <span className="ml-auto text-xs text-muted-foreground">{t("roles.branch_manager")}</span>}
                  {role === 'branch_staff' && <span className="ml-auto text-xs text-muted-foreground">{t("roles.branch_staff")}</span>}
                  {role === 'user' && <span className="ml-auto text-xs text-muted-foreground">{t("roles.user")}</span>}
                </button>
              ))}
            </div>
            <div className="flex gap-3">
              ...
              <button onClick={() => setSelectedUser(null)} className="px-5 py-3 border border-border rounded-xl hover:bg-muted transition-colors font-medium">{t("cancel")}</button>
            </div>
          </div>
        </div>
      )}
