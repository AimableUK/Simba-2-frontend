...
import {
  ...
  MessageSquare, Image as ImageIcon, Settings, Menu, X,
  ShoppingBag, LogOut, Bell, ChevronRight
} from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useAdminSocket({
    onNewOrder: (data) => {
      ...
      toast(`🛒 New order: ${data.orderNumber}`, { description: `RWF ${data.total?.toLocaleString()}` });
    },
    onNewContact: (data) => {
      setNotifications((n) => n + 1);
  ...
  useEffect(() => {
    ...
    if (!isPending && session?.user && !['admin', 'super_admin', 'poster'].includes(role)) {
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 py-4 overflow-y-auto">
          {NAV_ITEMS.filter(item => {
            if (item.href === 'users' || item.href === 'settings') return role === 'super_admin';
            if (item.href === 'products' || item.href === 'blogs') return ['poster','admin','super_admin'].includes(role);
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 bg-background/80 backdrop-blur border-b border-border flex items-center justify-between px-4 h-14">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href={`/${locale}`} className="text-sm text-muted-foreground hover:text-primary transition-colors hidden sm:block">
              ← View Store
            </Link>
          </div>
        </header>
