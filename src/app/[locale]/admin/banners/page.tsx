...
export default function AdminBannersPage() {
  ...
  const [form, setForm] = useState({ title: '', subtitle: '', image: '', link: '', isActive: true, sortOrder: 0 });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Banner created!'); closeForm(); qc.invalidateQueries({ queryKey: ['admin-banners'] }); },
    onError: (err: any) => toast.error(err?.response?.data?.message || 'Failed'),
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => bannerApi.update(id, data),
    onSuccess: () => { toast.success('Banner updated!'); closeForm(); qc.invalidateQueries({ queryKey: ['admin-banners'] }); },
    onError: (err: any) => toast.error(err?.response?.data?.message || 'Failed'),
  });
  ...
  const openEdit = (banner: Banner) => {
    setEditing(banner);
    setForm({ title: banner.title, subtitle: banner.subtitle || '', image: banner.image, link: banner.link || '', isActive: banner.isActive, sortOrder: banner.sortOrder });
    setShowForm(true);
  };

  const closeForm = () => { setShowForm(false); setEditing(null); setForm({ title: '', subtitle: '', image: '', link: '', isActive: true, sortOrder: 0 }); };
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4">
          {banners?.map((banner: Banner) => (
            <div key={banner.id} className="bg-card border border-border rounded-2xl overflow-hidden flex flex-col sm:flex-row">
              <div className="relative h-36 sm:w-64 shrink-0 bg-muted">
                <Image src={getImageUrl(banner.image)} alt={banner.title} fill className="object-cover" sizes="256px" />
              </div>
              <div className="flex-1 p-5 flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-bold text-base">{banner.title}</h3>
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full shrink-0 ${banner.isActive ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-muted text-muted-foreground'}`}>
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Sort order: {banner.sortOrder}</p>
                </div>
                <div className="flex gap-2 mt-4">
                  <button onClick={() => openEdit(banner)} className="flex items-center gap-1.5 text-sm text-primary border border-primary/30 px-3 py-1.5 rounded-lg hover:bg-primary/5 transition-colors">
                    <Edit className="h-3.5 w-3.5" /> Edit
                  </button>
                  <button onClick={() => deleteMutation.mutate(banner.id)} className="flex items-center gap-1.5 text-sm text-destructive border border-destructive/30 px-3 py-1.5 rounded-lg hover:bg-destructive/5 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={closeForm}>
          <div className="bg-card border border-border rounded-2xl p-6 max-w-lg w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {[
                { key: 'title', label: 'Title *', placeholder: 'Fresh Groceries...', required: true },
                { key: 'subtitle', label: 'Subtitle', placeholder: 'Shop the freshest...', required: false },
                { key: 'image', label: 'Image URL *', placeholder: 'https://...', required: true },
                { key: 'link', label: 'Link URL', placeholder: '/shop', required: false },
