    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-4 py-12">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold mt-6 mb-1">{t('forgotPassword')}</h1>
          <p className="text-muted-foreground text-sm">Enter your email to receive a reset link</p>
        </div>

        {sent ? (
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-2xl p-6 text-center">
            <p className="font-semibold text-green-700 dark:text-green-400 mb-2">Email Sent!</p>
            <p className="text-sm text-muted-foreground">{t('resetSent')}</p>
            <Link href={`/${locale}/auth/sign-in`} className="mt-4 inline-block text-primary hover:underline text-sm">
              ← {t('signIn')}
            </Link>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Link href={`/${locale}/auth/sign-in`} className="flex items-center justify-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors">
