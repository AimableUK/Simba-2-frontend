...
export function FeaturedProducts() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <SectionHeader title={t('featured')} desc={t('featuredDesc')} href={`/${locale}/shop?featured=true`} />
  ...
  return (
    <section className="bg-muted/50 py-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <SectionHeader title={t('topProducts')} desc={t('topProductsDesc')} href={`/${locale}/shop?sort=salesCount`} />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
          {isLoading
            ? Array.from({ length: 10 }).map((_, i) => <ProductCardSkeleton key={i} />)
            : products?.map((p: any) => <ProductCard key={p.id} product={p} />)}
...
export function RecommendedProducts() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <SectionHeader title={t('recommended')} desc={t('recommendedDesc')} href={`/${locale}/shop`} />
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
        {isLoading
          ? Array.from({ length: 10 }).map((_, i) => <ProductCardSkeleton key={i} />)
          : products?.map((p: any) => <ProductCard key={p.id} product={p} />)}
...
export function CategoryGrid() {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
      <SectionHeader title={t('categories')} desc={t('categoriesDesc')} href={`/${locale}/shop`} />
        ...
        {isLoading
          ...
          : categories?.slice(0, 8).map((cat: any, i: number) => (
              <Link key={cat.id} href={`/${locale}/shop?category=${cat.slug}`}
                ...
                <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center text-xl', `bg-gradient-to-br ${COLORS[i % COLORS.length]}`)}>
                  {cat.image ? (
                    <Image src={getImageUrl(cat.image)} alt={cat.name} width={32} height={32} className="object-cover rounded-lg" />
                  ) : (
                    <span>🛒</span>
                  )}
                </div>
