...
import {
  ShoppingCart, Search, Menu, X, Sun, Moon, Globe, Heart,
  User, LogOut, Settings, Package, ChevronDown, Bell,
} from 'lucide-react';
...
export function Navbar() {
  ...
  const { searchOpen, mobileMenuOpen, toggleSearch, openMobileMenu, closeMobileMenu } = useUIStore();
  ...
  const isAdmin = user?.role && ['admin', 'super_admin', 'poster'].includes(user.role);
          ...
          <div className="flex items-center h-16 gap-4">
            ...
            <Link href={`/${locale}`} className="flex items-center gap-2 flex-shrink-0">
              ...
              <div className="hidden sm:block">
                <div className="font-bold text-base leading-tight text-foreground">Simba</div>
                  ...
                  <AnimatePresence>
                    {userOpen && (
                      ...
                      >
                        ...
                        <div className="py-1">
                          <Link href={`/${locale}/account`} onClick={() => setUserOpen(false)}
                            className="flex items-center gap-3 px-4 py-2 text-sm hover:bg-accent transition-colors">
                            <User className="w-4 h-4 text-muted-foreground" /> {t('account')}
                          </Link>
                          <Link href={`/${locale}/account/orders`} onClick={() => setUserOpen(false)}
                            className="flex items-center gap-3 px-4 py-2 text-sm hover:bg-accent transition-colors">
                            <Package className="w-4 h-4 text-muted-foreground" /> {t('orders')}
                          </Link>
                          {isAdmin && (
                        ...
                        <div className="py-1 border-t border-border">
                          <button
                            onClick={async () => { setUserOpen(false); await signOut(); router.refresh(); }}
            ...
            >
              <div className="flex items-center justify-between p-4 border-b border-border">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-foreground">Simba Super Market</span>
                </div>
                <button onClick={closeMobileMenu} className="p-1 rounded-lg hover:bg-accent">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <nav className="flex-1 overflow-y-auto p-4 space-y-1">
                ...
                <div className="pt-2">
                  ...
                  {categories?.map((cat: any) => (
                    <Link key={cat.id} href={`/${locale}/shop?category=${cat.slug}`} onClick={closeMobileMenu}
              ...
              <div className="p-4 border-t border-border space-y-2">
                {user ? (
                  <>
                    ...
                    <button
                      onClick={async () => { closeMobileMenu(); await signOut(); router.refresh(); }}
                  ...
                  <Link href={`/${locale}/auth/sign-in`} onClick={closeMobileMenu}
